...
ext_help = (
    "if specified, will be used as the output file extension instead of " "FORMAT"
)
...
nfp_group.add_argument(
    "--skip-resize", "-s", help=skip_help, action="store_true", default=False
)
nfp_group.add_argument(
    "--resize-width",
    "-w",
...
im_group.add_argument(
    "--format", "-f", help=format_help, metavar="FORMAT", dest="f_format", default="PNG"
)
...
for file in args.files:
    ...
    if ext.upper() == ".NFP":
        ...
        code = pastebin.upload(nfp_file)
        if code.startswith("FAILURE"):
            print(code)
            _ = input()
        else:
            pyperclip.copy(code)
